...
describe('run-unit-parallel.sh failure-log contract (d)', () => {
  ...
  it('prints loud stderr banner with absolute failure-log path on failure', () => {
    ...
    // Git Bash renders a Windows temp directory as /tmp/... while Node sees
    // C:\Users\...\Temp\.... Either is an absolute path users can pass to
    // the active shell, so validate absoluteness plus the stable suffix.
    const pathLine = r.stderr
      .split(/\r?\n/)
      .map(line => line.trim())
      .find(line => /[\\/]\.context[\\/]test-failures\.log$/.test(line));
    expect(pathLine).toMatch(/^(?:\/|[A-Za-z]:[\\/])/);
  });
