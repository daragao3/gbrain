/**
 * #774 NAV-1/NAV-2 path-containment guard (`isPathInside` in path-confine.ts).
 *
...
describe('isPathInside path containment (#774 NAV-1/NAV-2)', () => {
  ...
  test('accepts a direct child', () => {
    expect(isPathInside(join(root, 'page.md'), root)).toBe(true);
  });

  test('accepts a nested descendant (the case a hardcoded "/" broke on win32)', () => {
    expect(isPathInside(join(root, 'wiki', 'notes', 'page.md'), root)).toBe(true);
  });

  test('rejects the parent directory', () => {
    expect(isPathInside(parse(root).dir, root)).toBe(false);
  });

  test('rejects a path escaping upward', () => {
    expect(isPathInside(join(root, '..', 'outside', 'page.md'), root)).toBe(false);
  });

  test('rejects a sibling sharing the root as a string prefix', () => {
    expect(isPathInside(root + '-evil', root)).toBe(false);
    expect(isPathInside(join(root + '-evil', 'page.md'), root)).toBe(false);
  });

  test('rejects a sibling whose name extends the last segment', () => {
    expect(isPathInside(join(parse(root).dir, 'repository', 'page.md'), root)).toBe(false);
  });
