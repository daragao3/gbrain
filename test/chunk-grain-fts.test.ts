...
// ONE engine for the whole file. Each of the three engine-backed describes
// below used to build its own, so the file paid three cold initSchema() runs
// — ~65s apiece on Windows, where PGLite replays 121 migrations through WASM.
// The describes need clean, differently-seeded data rather than clean
// *schema*, so they reset user data (TRUNCATE) and re-seed instead.
