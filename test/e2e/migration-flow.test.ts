 ...
 *
 * Exercises the full orchestrator from Phase A (in-process schema apply)
 * through Phase G (completed.jsonl append),
...
import { makeGbrainShim, type GbrainShim } from '../helpers/gbrain-shim.ts';
import { PGLITE_BOOTSTRAP_MS } from '../helpers/pglite-spawn-budget.ts';
...
let gbrainShim: GbrainShim | undefined;

// Module-level PATH shim. Phase A runs in-process, but Phase B shells out to
// `gbrain jobs smoke`. On source-install test envs there is no `gbrain` on
// PATH. The shared helper emits a .cmd shim on Windows and a shell shim on
// POSIX, then prepends it with the platform's PATH delimiter. Running at
// module init guarantees the shim exists before Bun loads described blocks.
if (!SKIP) {
  origPath = process.env.PATH;
  gbrainShim = makeGbrainShim('gbrain-e2e-bin-');
  process.env.PATH = gbrainShim.pathValue;
  console.log('[migration-flow.e2e] shim installed at', gbrainShim.binDir, 'PATH prepended');
}
...
function restoreHomePath() {
  ...
  if (gbrainShim) process.env.PATH = gbrainShim.pathValue;
  else if (origPath === undefined) delete process.env.PATH;
  ...
  test('host rewrite: builtin handlers auto-rewritten, non-builtins queued as JSONL TODOs', async () => {
    ...
    try {
      ...
      for (const todo of todos) {
        ...
        // manifest_path is built with path.join and uses native separators.
