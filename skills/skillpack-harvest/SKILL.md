...
- The user developed a skill in their OpenClaw deployment or another host
  fork and wants other gbrain clients to be able to use it
...
- Which host repo is the source? (Path to repo root, not to the skill
  directory — e.g. `~/git/my-openclaw`, not `~/git/my-openclaw/skills/foo`.)
...
1. **Fork-specific names → generic phrasing**
   ...
   - Other private deployment names → same treatment
   - Personal first names → `the user` / `you` / a generic placeholder
...
- Privacy linter runs against `~/.gbrain/harvest-private-patterns.txt`
  plus built-in private-name, email, and Slack-channel checks
...
should be able to see WHY the lint was bypassed (e.g. "the matched
private-name token appears only in a citation — verified manually").
