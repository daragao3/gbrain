...
- **YAML frontmatter fences: never LF-only.** `content.match(/^---\n…/)` cannot match a file
  ...
  since normalizing would corrupt every offset. When the offset contract forbids normalizing
  the HAYSTACK, normalize the NEEDLE: `findTarget` in that file matches an edit target by
  trying it as authored, then retrying it rewritten in the body's detected ending, so the body
  and every offset into it stay untouched. A raw `indexOf` there silently rejected every
  multi-line `replace`/`delete` as `target_not_found` on CRLF.
