...
Contract-first: `src/core/operations.ts` defines ~90 shared operations (v0.29 adds `get_recent_salience`, `find_anomalies`, `get_recent_transcripts`; v0.42.43.0 adds `volunteer_context` — push-based context, see `docs/guides/push-context.md`; v0.42.87.0 adds `put_page_conditional` — conditional page writes, see the invariant below). CLI and MCP
...
- **YAML frontmatter fences: never LF-only.** `content.match(/^---\n…/)` cannot match a file
  ...
  installer default) makes EVERY checked-out `SKILL.md` CRLF, so this fires for the whole
  skills tree, and it is an identity transform on POSIX so Linux CI can never catch it.
  `skillsDir` is a runtime parameter — gbrain parses `SKILL.md` files it does not own — so
  `.gitattributes` cannot fix this; the parsers must be tolerant. Canonical form is
  `src/core/skill-frontmatter.ts`: `content.match(/^---\r?\n([\s\S]*?)\r?\n---/)`. Prefer
...
- **The repo's own shell scripts: LF-pinned, and invoked through `bash`.** Complementary to
  ...
  `*.sh text eol=lf`, without which `core.autocrlf=true` checks every script out CRLF and a
  strict bash dies on the trailing `\r`; (2) every `package.json` script that runs a `.sh`
  MUST be written `bash scripts/foo.sh`, never the bare path — bun on Windows will not start
  a file from its shebang, so a bare path exits instantly with "command not found" and the
  check reports failure without having read a line. A bare path works on Linux and macOS, so
...
- **Dependency security floors move as one compatible unit.** When a fixed transitive
...
- **`link_resolution.entity_dirs` is load-bearing SAFETY, not a recall knob.** It reads as
  coverage ("which top-level dirs produce typed edges"), but `runAutoLink` — the only
  automatic link-deletion path — deletes every reconcilable edge whose key is missing from
  the freshly-extracted desired set, and that removal loop is NOT gated on the desired set
  being empty. A prefix that stops being declared stops being extractable, so its edges are
  hard-deleted on the next local `put_page`, and `links` has no tombstone column. Narrowing
...
- **Never hardcode `origin` as a push remote.** Every push site takes a CALLER-SUPPLIED repo —
  `sources.local_path` (`sources add --path` fills it with the user's OWN unowned tree;
  `harden --all` sweeps them) and the skillpack registry root (defaults to CWD) — and those are
  routinely fork-shaped, where `origin` is an upstream the user must not write to. Go through
  `resolvePushRemote` (`src/core/git-remote.ts`): `branch.<b>.pushRemote` → `remote.pushDefault`
  → `branch.<b>.remote` → `branch.{main,master}.remote` → sole remote → `origin` only if it
  exists, each value filtered against `git remote` (a value may be a URL); null → refuse.
  Paired READS move with the push or the fix defeats itself: a rebase-retry's `pull --rebase`,
  any `<remote>/<branch>` comparison. Generated shell mirrors the chain (`brain_remote`). Pinned
  by `test/git-push-remote.serial.test.ts` (real bare upstream+fork repos, so a regression is
  commits in the wrong repo, not an argv assertion). Standalone fetch paths still say `origin`
  (`TODOS` v0.42.91.0).
...
- **Page writes are last-writer-wins unless they opt out.** `put_page` overwrites, which is
  correct for a sole writer and lossy for concurrent ones — there is no trash can for the
  content and links it removes. A caller that can race uses `put_page_conditional`
  (`create_only`, or `compare_and_swap` at an `expected_revision`); conflicts come back as
  typed results (`already_exists`, `revision_mismatch`) carrying the revision actually found
  and never overwrite. `pages.revision` is maintained by the `bump_page_revision_trg` trigger,
  NOT by application code, and it counts unconditional writers too — that is precisely what
  lets a conditional write detect a plain `put_page` that landed underneath it. Never derive
  the next revision in TypeScript. Migration v126 `page_revision_cas` adds the column
  (`DEFAULT 1`) and the trigger; `src/schema.sql` carries both for fresh installs, so the two
  must move together like any other engine-parity change. The op is agent-facing: it carries
  no `cliHints`, so it appears in `gbrain --tools-json` and over MCP but has no subcommand.
