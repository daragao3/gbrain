...
| `bun run test:serial` | Just the `*.serial.test.ts` set (cross-file-contention quarantine; one bun process per file for true module-registry isolation). | ~1s for most files, but a few migration-heavy ones dominate: `migrate-engine-page-copy-failure` measured 279s on a loaded Windows box, and `pglite-snapshot-file-seeding` builds its fixture first and is given a 900s cap | Debugging a specific quarantined file. |
...
The outer wrapper has two independent bounds. `GBRAIN_TEST_SHARD_TIMEOUT` is the whole-shard wallclock cap: it remains 1,500 seconds on POSIX. On Windows it is derived — files-per-shard × `GBRAIN_TEST_SECONDS_PER_FILE` (default 30), with a 1,500-second floor — for **every** shard count, not only the single shard Windows picks for itself. A fixed cap is a per-file budget in disguise, so it rots whenever the suite size or the host speed moves: 1,500 seconds across a 275-file shard is 5.5 seconds per file, and a Windows unit file that constructs one PGLite engine pays roughly 65 seconds of cold `initSchema()` replay before its first assertion. That is why a healthy `--shards 4` Windows run used to report `WEDGED` on all four shards. An explicit environment override remains authoritative. `GBRAIN_TEST_SHARD_STALL_SECONDS` is a no-log-growth watchdog: 600 seconds on POSIX, and on Windows the shard runner's own effective chunk cap plus 300 seconds (`0` disables it). It has to clear the chunk cap there, because Bun's reporter writes nothing until its process exits unless a test fails or the code under test logs — `test/scripts/check-test-isolation.test.ts` and `test/child-worker-supervisor.test.ts` each sit at 28 bytes (the version banner) for their whole run. A chunk of four healthy-but-slow quiet files is therefore silent for longer than 600 seconds, and a shorter window kills the whole shard for passing. The wrapper reads the cap from `run-unit-shard.sh --print-chunk-cap` rather than keeping its own copy of the chunk constants. The two bounds then divide cleanly: the chunk cap bounds a hung Bun process, and a shard silent for longer than one whole chunk is a wedged shell. `run-unit-shard.sh` keeps its natural-completion marker path private from descendants, and the wrapper requires that attestation before accepting rc 0 with zero reported failures. Helper `STALLED`/`WEDGED` evidence remains fail-closed even if marker creation raced helper termination; rc 0 without a marker is reported as `UNATTESTED`. A genuine outer wallclock expiry is reported as `WEDGED`; a no-log expiry is reported as `STALLED`.

### Chunking a shard's file list (`GBRAIN_TEST_CHUNK_SIZE`)

`bun test` can die mid-run *before* printing its summary block — on Windows this shows up as error 127 (`ERROR_PROC_NOT_FOUND`, a WASM/native load failure, not shell "command not found"). When that happens the pass/fail totals for every file in that invocation are lost. The Windows defaults therefore use one shard for bounded aggregate memory, split its files into groups of four, and wrap each Bun process in a derived chunk cap. POSIX keeps chunking and the chunk cap disabled by default.

| Platform | `GBRAIN_TEST_CHUNK_SIZE` default | `GBRAIN_TEST_CHUNK_TIMEOUT` default |
|---|---:|---:|
| POSIX Bun (Linux, macOS) | `0` (off) | `0` (off) |
| Windows Bun, including when hosted by WSL/MSYS/MINGW/CYGWIN | `4` | chunk size × `GBRAIN_TEST_CHUNK_SECONDS_PER_FILE` (default 600) = `2400` seconds |

The chunk cap is derived from the **effective** chunk size for the same reason the shard cap is derived from the file count: a fixed number is a per-file budget in disguise. The previous flat 300 seconds was 75 seconds per file across a four-file chunk, while `test/sync-monorepo.test.ts` measures 389 seconds by itself on an idle Windows box — so every chunk containing that file was killed and lost the totals for all four of its files on every run, which is indistinguishable from the hang the cap exists to catch. Raising `GBRAIN_TEST_CHUNK_SIZE` now raises the cap with it.

All three overrides accept bounded non-negative Bash integers; `0` disables that bound. Each chunk runs as a separate `bun test`, and `run-unit-parallel.sh` sums all emitted Bun summaries. The chunk cap first sends TERM and then, after a fixed five-second grace, force-kills the exact owned chunk command; this keeps TERM-resistant Bun/native work within a real wallclock bound without enumerating or targeting unrelated processes. The cap deliberately does **not** pass `--preserve-status` to `timeout(1)`: that flag reports the killed command's own status (143 on TERM, 137 on KILL) instead of GNU timeout's 124, and a 143 was not in the diagnosis table, so a capped chunk failed the shard without naming the files that ate the cap. An rc 124 chunk is reported as `CHUNK N STALLED`, an rc 137 or 143 chunk as `CHUNK N FORCE-KILLED`, and an rc 127 chunk as `CHUNK N CRASHED`; all diagnostics name every file in the affected chunk. These inner diagnoses remain distinct from the outer whole-shard `STALLED`/`WEDGED` sentinels. `GBRAIN_TEST_SHARD_COMPLETED_FILE` is the supervisor-only natural-completion marker and is touched only after a clean shard run.
...
**Unsetting `GBRAIN_HOME` makes a `saveConfig` call machine-global.** `configDir()` re-reads the var at call time and falls back to `homedir()`, so a test that clears it is writing the developer's real `~/.gbrain/config.json`, not a sandbox. `saveConfig` fails closed on the shape that silently destroys a brain — no `GBRAIN_HOME` plus a `database_path` inside `tmpdir()` (compared after `realpathSync.native` canonicalization, so a Windows 8.3 short name doesn't slip past) or carrying the `gbrain-migrate-target-` mkdtemp prefix — and throws `UnsafeConfigWriteError` instead of writing. Keep `GBRAIN_HOME` pointed at a temp dir for anything that saves config; clear it only to assert the fallback itself, and redirect `USERPROFILE`/`HOME` to a temp dir when you do, so a regression cannot reach the real config. `GBRAIN_ALLOW_TEMP_BRAIN=1` opts out where a test genuinely needs the pre-guard behavior.
...
- `test/dependency-security.test.ts` — dependency-policy regression guard: asserts the manifest keeps the MCP SDK and transitive overrides at the audited floors, and that `bun.lock` resolves those floors or newer.
