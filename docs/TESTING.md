...
**Unsetting `GBRAIN_HOME` makes a `saveConfig` call machine-global.** `configDir()` re-reads the var at call time and falls back to `homedir()`, so a test that clears it is writing the developer's real `~/.gbrain/config.json`, not a sandbox. `saveConfig` fails closed on the shape that silently destroys a brain — no `GBRAIN_HOME` plus a throwaway `database_path`, meaning either inside `tmpdir()` (compared after canonicalizing both sides, so a Windows 8.3 short name cannot slip past) or carrying the `gbrain-migrate-target-` scratch prefix — and throws instead of writing. Keep `GBRAIN_HOME` pointed at a temp dir for anything that saves config; clear it only to assert the fallback itself, and redirect `USERPROFILE`/`HOME` to a temp dir when you do, so a regression cannot reach the real config. `GBRAIN_ALLOW_TEMP_BRAIN=1` opts out where a test genuinely needs the pre-guard behavior.
