...
import { requireBearerAuthRetryable } from '../core/retryable-bearer-auth.ts';
...
<div class="prompt">&ldquo;Give me the GBrain admin login link&rdquo;</div>
  ...
  app.post('/mcp', requireBearerAuthRetryable({ verifier: oauthProvider }), async (req: Request, res: Response) => {
  ...
  app.post(
    '/ingest',
    ingestRateLimiter,
    requireBearerAuthRetryable({ verifier: oauthProvider, requiredScopes: ['write'] }),
